...
export class LoginComponent implements OnInit {
  ...
  public viewRecoverAccount(){
    this.router.navigate(['/sistemAnimalscouple/recoverandenabled/account']) .then(() => { window.location.reload(); });
  }
      ...
      .subscribe(
        (data: any) => {
          ...
          if (data != null) {
            ...
            localStorage.setItem('id_user_person', String(data.persona.id_persona));
  ...
  public validUserAndPassword(){
    let valorView = false;
    if(this.user.usuario != null && this.user.usuario != undefined && this.user.usuario != '' && this.user.contrasenia != null && this.user.contrasenia != undefined && this.user.contrasenia != ''){
      this.signInUser();
    }else{
      valorView = true;
    ...
    if(valorView){
      if(this.user.usuario === null || this.user.usuario === undefined || this.user.usuario === ''){
